...
from PyQt6.QtCore import (
    Qt, QMimeData, QPoint, QRect, QTimer, pyqtSignal, QThread,
)
...
def generate_theme_preview(theme: dict, output_path: str) -> str:
    ...
    if t_type == "bible":
        demo_text = "Ioan 3:16"
        demo_sub  = "Fiindcă Dumnezeu\naşa a iubit lumea..."
        vz = l.get("verse_zone", {})
        rz = l.get("ref_zone",   {})

        if vz:
            vx = int(vz.get("x", 10) / 100 * w)
            vy = int(vz.get("y", 20) / 100 * h)
            vw = int(vz.get("w", 80) / 100 * w)
            vh = int(vz.get("h", 50) / 100 * h)
            if t.get("text_shadow", "true") == "true":
                so = max(1, int(2 * scale))
                painter.setPen(QColor(0, 0, 0, 160))
                painter.drawText(
                    QRect(vx + so, vy + so, vw, vh),
                    Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop |
                    Qt.TextFlag.TextWordWrap, demo_sub)
            painter.setPen(color)
            painter.drawText(
                QRect(vx, vy, vw, vh),
                Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop |
        ...
        if rz:
            ref_size  = max(6, int(l.get("ref_font_size", 24) * scale))
            ref_color = QColor(l.get("ref_color", "#aaaaaa"))
            painter.setFont(QFont(family, ref_size))
            painter.setPen(ref_color)
            rx = int(rz.get("x", 60) / 100 * w)
            ry = int(rz.get("y", 75) / 100 * h)
            rw = int(rz.get("w", 35) / 100 * w)
            rh = int(rz.get("h", 15) / 100 * h)
            painter.drawText(
                QRect(rx, ry, rw, rh),
            ...
            painter.setFont(font)
            painter.setPen(color)
            painter.drawText((w - fm.horizontalAdvance(demo_text)) // 2,
                             h // 2, demo_text)
    else:
        demo_text = "Doamne, Tu ești"
        demo_sub  = "lumina mea"
        fm  = QFontMetrics(font)
        tw1 = fm.horizontalAdvance(demo_text)
        tw2 = fm.horizontalAdvance(demo_sub)
        lh  = int(fm.height() * 1.4)
        x1  = (w - tw1) // 2
        x2  = (w - tw2) // 2
        y1  = h // 2 - lh // 2
        y2  = h // 2 + lh // 2
        ...
        if t.get("text_shadow", "true") == "true":
            so = max(1, int(2 * scale))
            painter.setPen(QColor(0, 0, 0, 160))
            painter.drawText(x1 + so, y1 + so, demo_text)
            painter.drawText(x2 + so, y2 + so, demo_sub)

        painter.setPen(color)
        painter.drawText(x1, y1, demo_text)
        painter.drawText(x2, y2, demo_sub)
...
class ThemeVisualEditor(QMainWindow):
    ...
    def _build_overlay_props(self):
        ...
        self.o_transition.addItems([
            "instant", "fade", "crossfade", "slide_left", "zoom_in"])
        self.o_transition.setCurrentIndex(2)
    ...
    def _load_theme(self):
        ...
        bg_map = {"color": 0, "gradient": 1, "image": 2,
                  ...
                  "transparent": 6, "animated_gradient": 7}
...
class ThemeCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if bg_type == "transparent":
            csz = 20
            for cy in range(0, ch, csz):
                for cx in range(0, cw, csz):
            ...
            gdir = bg.get("grad_dir", "Sus→Jos")
            if gdir == "Radial":
                grad = QRadialGradient(cw / 2, ch / 2, max(cw, ch) / 2)
            elif "Stânga" in gdir or "Left" in gdir:
                grad = QLinearGradient(0, 0, cw, 0)
            elif "Diagonal" in gdir:
                grad = QLinearGradient(0, 0, cw, ch)
            else:
                grad = QLinearGradient(0, 0, 0, ch)
            grad.setColorAt(0, c1); grad.setColorAt(1, c2)
            painter.fillRect(0, 0, cw, ch, QBrush(grad))

        else:
            painter.fillRect(0, 0, cw, ch,
                             QColor(bg.get("color", "#000000")))
            bg_img = bg.get("image", "")
        ...
        text_color   = QColor(t.get("text_color",  "#ffffff"))
        family       = t.get("font_family", "Arial")
        # Use setPixelSize() to match Canvas 2D pixels exactly
        font_size_px = max(4, int(int(t.get("font_size", 48)) * scale))
        bold         = t.get("font_bold",   "true") == "true"
        italic       = t.get("font_italic", "false") == "true"
        shadow       = t.get("text_shadow", "true") == "true"
        outline_px   = int(t.get("outline_width", 0))
        outline_c    = QColor(t.get("outline_color", "#000000"))
        lspacing     = float(t.get("line_spacing", 1.4))
        ...
        if t_type == "bible":
            demo_verse = ("Fiindcă Dumnezeu aşa a iubit\n"
                          "lumea, că a dat pe singurul Lui Fiu...")
            demo_ref   = "Ioan 3:16"

            vz = l.get("verse_zone", {"x": 10, "y": 15, "w": 80, "h": 55})
            rz = l.get("ref_zone",   {"x": 60, "y": 75, "w": 35, "h": 15})

            vx = int(vz["x"] / 100 * cw); vy = int(vz["y"] / 100 * ch)
            vw = int(vz["w"] / 100 * cw); vh = int(vz["h"] / 100 * ch)

            # Zone outlines (dashed)
            painter.setPen(QPen(QColor(203, 166, 247, 170), 1,
                                Qt.PenStyle.DashLine))
            painter.setBrush(QBrush(QColor(203, 166, 247, 17)))
            ...
            if shadow:
                ...
                painter.drawText(
                    QRect(vx + so, vy + so, vw, vh),
                    Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop |
                    Qt.TextFlag.TextWordWrap, demo_verse)
            ...
            ref_size_px = max(6, int(l.get("ref_font_size", 24) * scale))
            ref_color   = QColor(l.get("ref_color", "#aaaaaa"))
            ref_font    = QFont(family)
            ref_font.setPixelSize(ref_size_px)

            rx = int(rz["x"] / 100 * cw); ry = int(rz["y"] / 100 * ch)
            rw = int(rz["w"] / 100 * cw); rh = int(rz["h"] / 100 * ch)

            painter.setPen(QPen(QColor(137, 220, 235, 170), 1,
                                Qt.PenStyle.DashLine))
            painter.setBrush(QBrush(QColor(137, 220, 235, 17)))
            ...
            total_h = line_h * len(demo_lines)
            start_y = (ch - total_h) // 2 + fm.ascent()
            ...
            for i, line in enumerate(demo_lines):
                ...
                if shadow:
                    so = max(1, int(3 * scale))
                    painter.setPen(QColor(0, 0, 0, 180))
                    painter.drawText(lx + so, ly + so, line)

                if outline_px > 0:
                    pen = QPen(outline_c)
                    pen.setWidth(max(1, int(outline_px * scale)))
                    painter.setPen(pen)
                    for dx in (-1, 0, 1):
                        for dy in (-1, 0, 1):
                            if dx or dy:
